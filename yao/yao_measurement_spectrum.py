...
def run_spectrum_for_different_domains(output_name, file_domain_index, unit_list_index):
    measurement_storage = MeasurementStorage('/Users/Frank/working-directory/ner-measurement-mentions/')
    measurement_dict = dict()

    # utility.create_parent_folder_if_needed_for_output_file(output_name)
    with open(file_domain_index) as file_domain_index_file:
        file_domain_dictionary = FileDomainDictionary(file_domain_index_file)
        ...
        while measurement_storage.has_next_measurement_object():
            ...
            for entry in measurement_object[1]['measurement']:
                ...
                if is_number(number):
                    number = float(number)
                    print(measurement_object[0])
                    # if isinstance(measurement_object[0], list):
                    #     print(measurement_object[0], 'skipped')
                    #     continue
                    if file_domain_dictionary.domain_of(measurement_object[0]) is None:
                        print(measurement_object[0], 'not in there')
                        continue
                    file_type = file_domain_dictionary.domain_of(measurement_object[0])
                    if file_type not in measurement_dict:
                        measurement_dict[file_type] = dict()
                    inner_dict = measurement_dict[file_type]
                    if unit not in inner_dict:
...
def main():
    ...
    # run_spectrum_for_different_domains('/Users/Frank/working-directory/spectrum/spectrum-grouped-by-domain/',
    #                                    '/Users/Frank/working-directory/fulldump/fulldump.txt',
